...
int main(int argc, char **argv)
{
  ...
  while(ros::ok() ){//&& !current_state.armed  ){// && current_state.mode != "OFFBOARD"){
        if( current_state.mode != "OFFBOARD" && (ros::Time::now() - last_request > ros::Duration(3.0))){ROS_INFO(current_state.mode.c_str());
   ...
   // local_pos_pub.publish(pose); ros::spinOnce(); rate.sleep(); ROS_INFO("Got data : %f, %f, %f", c_p.pose.position.x, c_p.pose.position.y, c_p.pose.position.z);} 
    ROS_INFO("take offs");
    pose.pose.position.x = 0; pose.pose.position.y = 0; pose.pose.position.z = 2.5;
    while(c_p.pose.position.z<2.4){local_pos_pub.publish(pose);ros::spinOnce();rate.sleep();ROS_INFO("Got data : %f", c_p.pose.position.z);}//vel_sp_pub.publish(vs);
    ROS_INFO("Landing");
